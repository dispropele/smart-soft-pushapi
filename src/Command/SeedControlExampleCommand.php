...
#[AsCommand(name: 'app:seed:control-example', description: 'Заполнение БД данными контрольного примера')]
class SeedControlExampleCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->title('Заполнение БД контрольным примером');
        ...
        // ?? Типы вставок (Таблица 1.13) ?ddfd?????????????????????????????????????
        $io->section('Вставки...');
        $itypes = [];
        ...
        $gtData = [
            ...
            ['Серьги','Серьги-пусеты',true,'gold_plating'],
            ['Серьги','Серьги с подвесками',true,'rhodium'],
        ...
        // [номер, дата, клиент, тариф, сумма, статус, закрыт?]
        $ticketData = [
            ['ЛБ-001','2023-10-01','Иванов Иван Иванович','Стандартный','25000.00','closed',true],
            ['ЛБ-002','2023-10-02','Петров Петр Петрович','Стандартный','12000.00','open',false],
            ['ЛБ-003','2023-10-05','Сидорова Анна Павловна','Льготный','45000.00','open',false],
            ['ЛБ-004','2023-10-07','Смирнова Елена Викторовна','Стандартный','8500.00','expired',true],
            ['ЛБ-005','2023-10-10','Васильев Сергей Андреевич','VIP','180000.00','open',false],
            ['ЛБ-006','2023-10-12','Федорова Дарья Алексеевна','Стандартный','32000.00','closed',true],
            ['ЛБ-007','2023-10-15','Семенова Инна Игоревна','Льготный','6000.00','open',false],
            ['ЛБ-008','2023-10-18','Макаров Денис Петрович','Стандартный','19500.00','expired',true],
            ['ЛБ-009','2023-10-20','Кузнецов Олег Игоревич','Срочный','60000.00','open',false],
            ['ЛБ-010','2023-10-22','Павлов Игорь Владимирович','VIP','95000.00','open',false],
        ];
        $tickets = [];
        foreach ($ticketData as [$num, $dateStr, $clientName, $tariffName, $amount, $status, $isClosed]) {
            ...
            $t->setTicketNumber($num)
                ...
                ->setInterestRate($tariff->getMonthlyRate())
        ...
        // [билет, вид изделия, проба, вес, оценка, статус]
        $itemsData = [
        ...
        foreach ($itemsData as [$ticketNum, $gtName, $stdKey, $weight, $estimate, $status]) {
            ...
            $item->setName($gtName)
                ...
                ->setScrapWeight($weight) // вес лома = вес изделия (без вставок)
            ...
            // Для реализации — устанавливаем цену продажи
            if ($status === PledgedItem::STATUS_FOR_SALE) {
                $item->setSoldPrice($estimate)
                    ->setPublishedAt(new \DateTime('2023-11-20'));
            }
            if ($status === PledgedItem::STATUS_REDEEMED) {
                $item->setRedemptionDate(new \DateTime('2023-11-10'));
            }
        ...
        $io->success('Контрольный пример успешно загружен!');
        $io->table(
